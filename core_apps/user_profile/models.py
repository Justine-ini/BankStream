...
class Profile(TimeStampedModel):
    ...
    title = models.CharField(_("Salutation"), max_length=5,
                             choices=Salutation.choices, default=Salutation.MR)
    ...
    employment_status = models.CharField(
        _("Employment Status"), max_length=20, choices=EmploymentStatus.choices, default=EmploymentStatus.SELF_EMPLOYED)
    ...
    signature_photo = CloudinaryField(
        _("Signature Photo"), blank=True, null=True)
    signature_photo_url = CloudinaryField(
        ...
        if self.id_issue_date and self.id_expiry_date:
            if self.id_expiry_date <= self.id_issue_date:
                raise ValidationError(
                    _("ID Expiry date must be later than the issue date")
                )
...
class NextOfKin(TimeStampedModel):
    ...
    def clean(self) -> None:
        ...
        if self.is_primary:
            primary_kin = NextOfKin.objects.filter(
                profile=self.profile, is_primary=True).exclude(pk=self.pk)
    ...
    def __str__(self) -> str:
        return f"{self.first_name} {self.last_name} -Next of kin for {self.profile.user.full_clean}"
